...
class TunnelAssetPriceCheckTestCase(TestCase):
    """Test suite for tunnel_asset_price_check Celery task
    ...
    def test_price_within_limits(self, mock_asset_handler_func, mock_tunnel_manager_class):
        ...
        # Act
        result = tunnel_asset_price_check(**self.test_kwargs)
    ...
    @patch("tunnels.tasks.price_checks.TunnelManager")
    @patch("tunnels.tasks.price_checks.asset_api_handler")
    def test_price_above_upper_limit(self, mock_asset_handler_func, mock_tunnel_manager_class):
    ...
    @patch("tunnels.tasks.price_checks.TunnelManager")
    @patch("tunnels.tasks.price_checks.asset_api_handler")
    def test_price_below_lower_limit(self, mock_asset_handler_func, mock_tunnel_manager_class):
    ...
    def test_float_price_values(self, mock_asset_handler_func, mock_tunnel_manager_class):
        ...
        mock_tunnel = MagicMock()
        mock_tunnel.asset.name = "Petrobras"
        mock_tunnel.upper_limit = 30.50
        mock_tunnel.lower_limit = 20.50
        mock_tunnel.asset_price_is_above_upper_limit.return_value = False
        mock_tunnel.asset_price_is_below_lower_limit.return_value = False
        mock_tunnel_manager_class.return_value = mock_tunnel
